...
- subtle な非executive BEC（語調を和らげた送金・認証情報要求型）は床落ちで取りこぼす。動的レッドチーム（A4）の敵対サンプル 4/4 が緑（score 39–49・閾値70）に着地。原因は intensity に下限が無く judge にも一般床も無いこと（friction=high が −2 で追い打ち）。これは自己改善ループ自身が炙り出した現在地で、行動レバー（送金・認証情報要求）に床を設ける方向で FPR 測定とセットで校正中。具体的手法は良性難ケースでの誤検知測定を経て決定する（床の最終決定は人間ゲート）。
