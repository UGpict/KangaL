...
### 圧縮 → 切り詰めの順序
`extractBody` 内で本文抽出直後に `compressUrls` を適用し、**その後** `parseMessage` が 8000 で切り詰める。逆順だと巨大URLが先頭を占有して本文が切り落とされる。実機（日本語の予約系サービスB）で URL 11個畳み・`truncated:false`・本文末尾まで可読を確認。
...
- 実機検証: 日本語メール2通（予約系サービスB / メールサービスC、ともに UTF-8 宣言）で正しくデコード・文字化けなし。shift_jis/iso-2022-jp で読むと化ける＝宣言 charset を正しく尊重している証左。
...
### 実機調査の発見（マーケメールA は検証サンプル不適・パーサ無罪）
マーケメールA（英語系送信元）は body 日本語が `?` 化していたが、生バイト診断（観察用 `scripts/probeRawCharset.ts`、未コミット）で **text/plain の宣言 charset = iso-8859-1、生バイトに 0x3F('?') が 798 個実在**と判明。送信元の生成系が日本語を iso-8859-1 へダウンコンバートする際に潰した壊れたデータで、**パーサは無罪**（snippet が正常なのは Gmail が text/html パートから生成しているため）。
- **HTMLフォールバックは今は不採用**（n=1 では「壊れた plain」がマーケメール特有か広範かが不明）。他の日本語メール（予約系サービスB / メールサービスC）は plain が正常 UTF-8 だったため、現時点で根拠なし。今後 `?` 密度の高い壊れ plain が頻発するなら「`?` 密度が閾値超で HTMLパート優先」を **G2.2** として切り出す（測ってから作る）。

### G2 完了条件（実機）— G2.1 検証で兼ねて達成
受信箱の一覧10件取得＋実メールの `Authentication-Results` が文字列で取得（マーケメールA で dkim/spf/dmarc を確認）。日本語本文の文字化けなしも 予約系サービスB / メールサービスC で確認。

> Pre-G4 公開前走査（全78コミット・読み取り）: 秘密・実メールデータ（本文/ヘッダ/Message-ID/Auth実値）は検出ゼロ。実機検証で言及していた受信箱由来の実在ブランド名のみ HEAD で匿名化（技術的意味は保持）。実害ほぼゼロのため履歴書き換え（コミット参照の喪失）は行わず、HEAD 修正＋本記録で公開可と判断。
...
## G3.1: 判定確定前の中間状態を「判定」として表示しない

実機で観察された不具合「判定開始から数秒、カードが詐欺(赤)⇔安全(緑)を往復してから安定する」の修正。**提示層のみ**で完結させる（judge ロジック・スコア・閾値70・調査ツールには一切触れない）。

### 真因 — 中間スコア仮説は構造的に不成立。真因はレスポンス競合
- 当初仮説「中間スコアが閾値70を跨ぐたびに描画される」は**構造的に成立しない**ことを調査で確定。`/api/judge` は `analyzeStructure→investigate→judge` を**サーバ側で完了してから単発の `Response.json`** を返す（ストリーミング/SSE/EventSource は不在、grep で確認）。クライアントは単発 await fetch → 単発描画。**ワイヤに中間スコアは存在しない**。
- 実際の往復の原因は**古い/並走レスポンスの上書き**。修正前 `judgeActive` には in-flight ガードが無く、メールを素早く2連続選択すると2本の `/api/judge` が並走し、各々 Gemini 由来で非決定的なスコアを返す。**last-write-wins** で、先に投げた古い選択のレスポンスが後から届くと現在の選択のカードを上書きして赤/緑が往復する。

### 修正 — seq ゲーティング ＋ AbortController（純関数 `judgeFlow.ts` + コンポーネント鏡像）
赤/緑カードは**終端状態**であり、確定（settled）シグナル前には絶対に出さない。確定前は中立の「調査中」のみ。
- **`src/lib/judgeFlow.ts`（純関数・node テスト可）**: 表示状態のたたみ込みを React/fetch から切り出した。`JudgeEvent`（select/settled/failed、各 `seq` 付き）の順序列を `flowFromEvents` で単一 `JudgePhase`（idle/investigating/settled/error）に畳む。規則は「`select` で `latestSeq` を更新し investigating に落とす。`settled`/`failed` は **`seq===latestSeq` の時だけ**適用、古い seq は捨てる」。これにより**現在の選択が settle するまで danger/safe フェーズが生成され得ない**ことを型と純関数で保証。`verdictCategory` は `degraded→degraded / score>=70→danger / <70→safe`（既存閾値を読むだけ・判定はしない）。
- **`InboxApp.tsx`（純関数の鏡像）**: `judgeSeqRef`（選択ごとに ++）・`abortRef`（新選択で前 fetch を abort）・`inflightIdRef`（同一 id の二重クリック無視）の3 ref。`beginSelection(id)` が「同一 in-flight なら null で無視／それ以外は前を abort し seq を ++」。`runJudge(msg, seq, signal)` は**カードを塗る全 setState を `seq===judgeSeqRef.current` でガード**＝純関数の seq ドロップ規則の実体。古いレスポンスは描画に反映されない。
- **コスト有界化（best-effort）**: 新選択で `AbortController.abort()` し並走 fetch を切る。1クリック=1判定に寄せ、審査期間の常時公開で無駄な Vertex 課金を抑える。**注意: クライアントの abort は fetch を切るだけで、サーバ route は signal を investigate/judge に伝播しない**ため Vertex 側の停止は best-effort。完全なサーバ側キャンセルは調査側変更になるためスコープ外。
- **AbortError は無言で握り潰す**（新選択が画面を所有するため、エラーカードを出さない）。Gmail の2段取得（detail→judge）は**同一 seq/signal で1選択**として扱い、detail fetch も superseded なら捨てる。

### 「調査中」表示 — 中立 zinc・中間スコアは出さない
- 確定前は `InvestigatingCard`（中立 zinc）。degraded の slate カード（判定保留）とは色で明確に区別（赤/緑とももちろん別）。
- **中間スコアの数値は出さない**（ワイヤに存在しないため出しようがなく、出すと「途中の数字」を判定と誤認させる）。代わりに D0.5 のツール名で「何を確認中か」を静的に並べる（既知手口照合・URL評価/Web Risk・送信者認証・ドメイン年齢・公的注意喚起）。動く進捗バーは作らない（サーバが段階を通知しないため嘘になる）。

### テスト — 確定前に赤/緑が生成されないことを純関数で保証
- `src/lib/__tests__/judgeFlow.test.ts`（10 件）: 空→idle・select→investigating（赤/緑なし）・現在の選択が settle した時のみ終端カテゴリ・**競合シナリオ**（A 選択→B 選択→A の danger 先着は破棄→B の safe で確定）・古い settled/failed の破棄・現在 settle 後の遅延古レスポンス無視・**任意の prefix で現在 settle 前に settled フェーズが出ないことの全網羅ループ**。
- サンプル／Gmail 双方が同一 `runJudge` 経由＝同じゲーティングで挙動一致。全テスト緑（371 passed / 6 skipped）・typecheck/lint クリーン。

### 完了条件の残（実機）— レース再現で真因と修正を同一操作で確認
- 修正**前**: メールを素早く2連続選択で赤⇔緑の往復が再現するか一度観察。
- 修正**後**: 同じ操作で往復ゼロ（確定までは「調査中」のみ、確定後に一度だけ終端カラー）。サンプル／Gmail 双方で確認。真因仮説の検証と修正の検証が同じ操作で済む。

### やらないこと（G3.1 範囲外）
judge ロジック・スコア・閾値・調査ツール・カード（VerdictCard）の変更／サーバ側リクエストキャンセル（route の signal 伝播）／中間スコアのストリーミング表示（ワイヤに存在しない）。
