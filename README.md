...
1. **何が新しいか** — 攻撃AIが見つけた検知器の死角を防御AIに還元し、検知器自体が自己改善する。攻防ループを「検知器の CI/CD」として設計した詐欺検知システム。
2. **どこが AI Agent か** — 防御AIがメールを6つの心理レバーに分解し、必要な調査ツール（事例DB照合 / Web Risk / RDAP / 送信者認証 / 公的アラート）を function-calling で自律選択する。
3. **安全設計** — 攻撃AIは完成詐欺文を生成・保存しない（6レバーの型のみ）。すり抜けた型は Firestore に蓄積され、次ラウンドで再検証される。
4. **今すぐ触れる** — https://kangal-649847191589.us-central1.run.app/ （ログイン不要・貼り付けて判定）。Cloud Run 本番稼働、公開APIのレート制限・AI停止時の決定論説明まで実装済み。
5. **実測と限界** — 外部 holdout は n=6（床60 で recall 4/6 [30–90%, Wilson 95% CI]）。できないことも数字の解像度で公開している（下記「測定の誠実さ」）。
...
- シークレットはリポジトリ・イメージに置かない（Cloud Run 実行 SA の ADC で Vertex を叩く。鍵は Secret Manager 経由）。
- **公開APIのコスト防御**: `/api/judge` は IP 単位＋インスタンス単位の2層レート制限。Cloud Run の静的天井（max-instances / concurrency / timeout）・予算アラートと合わせた多層防御。
- **外部AI出力の二重壁**: Gemini の構造化出力は responseSchema に加え、実行時の深い検証（enum 帰属・数値範囲・未知キー拒否）で受ける。不正な応答は安全側でなく**判定保留**に倒す（fail-open 封鎖）。
- **AI停止時も説明可能**: 説明生成の Gemini が落ちても、検出済みレバーと調査結果から「なぜ危ないか」を決定論的に組み立てて表示する。
